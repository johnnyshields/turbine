...
Turbine is a buffer allocator, not a runtime. It slots underneath an existing
async runtime or custom event loop as the buffer management layer. This guide
covers integration with specific runtimes. For general usage, configuration,
and tuning, see the [User Guide](guide.md).
...
|---|---|---|
| Buffer allocation | Heap box per I/O op | Bump alloc (~2 ns, constant) |
